...
{ ************************************************************************************************** }
{ }
{ Last modified: $Date:: 2011-12-29 00:21:39 +0000                                               $ }
{ Revision:      $Rev:: 86aeb2969014452af2fae18567917903a7527ac4                                 $ }
{ Author:        $Author:: outchy                                                                $ }
{ }
{ ************************************************************************************************** }

unit ExceptionDialogUnit;
...
begin
  with TJclEmail.Create do
    try
      ...
      Recipients.Add('artem-xp@ya.ru');
      Subject := 'AlphaSklad Exception';
...
begin
  with TSaveDialog.Create(Self) do
    try
      DefaultExt := '.log';
      FileName := ExtractFileName(Application.ExeName) + '-exception-' +
...
begin
  ...
  try
    CreateReport;
    SaveToLogFile(ExtractFileName(Application.ExeName) + '-exception-' +
      FormatDateTime('yyyy-mm-dd', Date) + '.log');
...
begin
  ...
  begin
    ...
    JclStackTrackingOptions := JclStackTrackingOptions +
