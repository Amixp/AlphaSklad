...
begin
  ...
  BtnPrintDoverka.Enabled:=true;
                                                            ...
                                                            { TODO : Добавить отображение выбранного клиента }
...
begin
 ...
 //  If WordApp. = 0 Then
  //
      ...
      L_Name := Trim(DataUnit.ADOPassports.FieldByName('Фамилия').AsString + ' ' +
        DataUnit.ADOPassports.FieldByName('Имя').AsString + ' ' + DataUnit.ADOPassports.FieldByName('Отчество').AsString);
      //****
     ...
     i := FormOptions.FormStorage1.StoredValues.Values['NumLetter'].Value;
      ...
      Begin
       ...
       // XL := CreateOLEObject('Excel.Application');
        ...
        XL.Range['B5', 'B5'].Value2 := DataUnit.ADOPassports.FieldByName('Серия').AsString;
        XL.Range['B6', 'B6'].Value2 := DataUnit.ADOPassports.FieldByName('Номер').AsString;
        XL.Range['B7', 'B7'].Value2 := DataUnit.ADOPassports.FieldByName('Кем выдан').AsString;
        ProgressForm.PrBar.StepIt;
        XL.Range['B8', 'B8'].Value2 := DataUnit.ADOPassports.FieldByName('Дата выдачи').AsDateTime; //SDay;
  ...
  Try
   ...
   //   mainform.SaveWordDoc(Text + S, FWordApp);
      ...
      Begin
        ...
        XL.Range['B5', 'B5'].Value := DataUnit.ADOPassports.FieldByName('Серия').AsString;
        XL.Range['B6', 'B6'].Value := DataUnit.ADOPassports.FieldByName('Номер').AsString;
        XL.Range['B7', 'B7'].Value := DataUnit.ADOPassports.FieldByName('Кем выдан').AsString;
        ProgressForm.PrBar.StepIt;
        XL.Range['B8', 'B8'].Value := DataUnit.ADOPassports.FieldByName('Дата выдачи').AsDateTime; //SDay;
...
TypeSaleProd:=  (sender as tbitbtn).Tag ;
  ...
  (sender as tbitbtn).enabled := False;
    ...
    Begin
      idx := i;
      ReadToVars;   { TODO : Locate - заменить! }
...
begin
  ...
  EdComment.Text := Comment;
      ...
      np := 0;
      Storage_fee[idx] := 0;
      If Not Datas.ADOFridges.Locate('Fridge', EdFridge.Text, [loCaseInsensitive]) Then { TODO : Locate заменить }
